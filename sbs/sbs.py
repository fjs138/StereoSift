...
def process_image_sbs(base_image, depth_map, method="mesh_warping", depth_scale=40, mode="parallel", depth_blur_strength=7):
    ...
    result = None
    # Follow the caller's selected accelerator (including Apple MPS) instead
    # of the module-level CUDA/CPU default.
    processing_device = base_image.device
    if method == "grid_sampling":
        result = process_image_sbs_grid_sampling(processing_device, base_image, depth_map, depth_scale, mode, depth_blur_strength)
    elif method == "mesh_warping":            
        result = process_image_sbs_mesh_warping(processing_device, base_image, depth_map, depth_scale, mode, depth_blur_strength)
